...
AWS_STORAGE_INPUT_BUCKET_NAME = "input-bucket-mavericks"
AWS_STORAGE_OUTPUT_BUCKET_NAME = "output-bucket-mavericks"
input_queue_url = "https://sqs.us-east-1.amazonaws.com/008112238476/image_processing_queue"
output_queue_url = "https://sqs.us-east-1.amazonaws.com/008112238476/output_queue"
    ...
    while True:
        ...
        if 'Messages' in response:
            for message in response['Messages']:
                ...
                result = subprocess.run(['python3', os.path.join(base_path, script_path), os.path.join(sqs_images_dir_path, file_name)], stdout=subprocess.PIPE)
                file_name, image_result = result.stdout.decode('utf-8').split(',')

                # Upload image to s3: Input bucket:
                s3_in_response = s3.put_object(Bucket=AWS_STORAGE_INPUT_BUCKET_NAME, Key=file_name,
                                               Body=encoded_data)
                ...
                key_val_string = '{}, {}'.format(file_name.split(".")[0], image_result)
                print(key_val_string)
                data = bytes(key_val_string, 'utf-8')
                s3_op_response = s3.put_object(Bucket=AWS_STORAGE_OUTPUT_BUCKET_NAME, Key=file_name.split(".")[0],
                                               Body=data)
